...
def Simulate2SpeciesSTED(STEDPOWER,NUMIM,NoiseList):
    ...
    savefolder = "Simulation_Cy3_{}Percent_2Species_PSD95Bassoon_NoiseonMixed".format(STEDPOWER)
    ...
    Overall_data = pd.DataFrame(
        columns=["Power",'image1', 'image2', 'resolution1', 'resolution2',"res_fraction1", "res_fraction2","squirrel_f1","squirrel_f2",
                 "squirrelsmooth_f1","squirrelsmooth_f2","Noise percent pixels","Noise number photons","Noise type"])
    ...
    # Plot the color-coded phasor plot and the line used for unmixing
            ...
            # Save the phasor plot
            fig4.savefig(os.path.join(savefolder,"Phasor_2species_Pair{}_Noise{}_{}_{}.pdf".format(Pair_id, noise[2], noise[0], noise[1])),transparent='True', bbox_inches="tight",dpi=900)
            # Remove the previous scatter plots from the figure
            pnplot.remove()
            p2plot.remove()
            ax_scatter.lines[-1].remove()
            ...
            ov_data.extend([res_fraction1* 20, res_fraction2* 20,squirrel_f1,squirrel_f2,squirrelsmoothf1[2],squirrelsmoothf2[2], noise[0], noise[1], noise[2]])
        ...
        # Create a composite image of the unmixed images and save it to a tiff file
            ...
            imagecomp=numpy.moveaxis(imagecomp,2,0)
            filenameout =os.path.join(savefolder, "{}_Predicted_Composite_{}Noise_{}_{}.tiff".format(Pair_id, noise[2], noise[0], noise[1]))
            imsave(file=filenameout, data=imagecomp.astype(numpy.uint16), composite=True, luts=("Cyan Hot","Magenta Hot"), pixelsize=(20E-3,20E-3))
            ...
            fig_im.savefig(os.path.join(savefolder,"Images_2species_Pair{}_Noise{}_{}_{}.pdf".format(Pair_id, noise[2], noise[0], noise[1])),transparent='True', bbox_inches="tight")
            plt.close(fig_im)
...
for power in Powerslist:
    stats=Simulate2SpeciesSTED(power[0],power[1],Noiselist)
    print(stats.shape)
