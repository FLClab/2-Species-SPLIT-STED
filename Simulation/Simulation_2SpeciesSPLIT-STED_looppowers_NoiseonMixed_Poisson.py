...
def Simulate3speciesLineControls(STEDPOWER, NUMIM, Noiselist):
    ...
    savefolder = "Simulation_Cy3_{}Percent_3Species_LineControls_PSD95Bassoon_NoisePoisson".format(STEDPOWER)
...
# Combine all possible pairs of images, perform unmixing and calculate metrics
    ...
    # Plot the color-coded phasor plot and the triangle used for unmixing
            ...
            # Save the phasor plot
            fig4.savefig(os.path.join(savefolder,"Phasor_3species_linefromcontrols_{}_Noise{}_{}_{}.pdf".format(Pair_id,noise[2],noise[0],noise[1])),transparent='True', bbox_inches="tight",dpi=900)
            # Remove the previous scatter plots from the figure
            pnscatter.remove()
            p2scatter.remove()
            p0scatter.remove()
